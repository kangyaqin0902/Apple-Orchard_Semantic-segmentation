...
class MoEFFNGating(nn.Module):
    ...
    def forward(self, x):
        ...
        features_for_loss = [f for f in x_downsample]
        features_for_loss.append(x)  # 加上 bottleneck 层的特征
        x = self.forward_up_features(x, x_downsample)
        x = self.up_x4(x)
        return x, features_for_loss
...
class SwinTransformerSys(nn.Module):
    ...
    # 修改：返回各层特征图以适配 Hierarchical Loss
    ...
    def forward(self, x):
        ...
        # 2. 格式转换：为了让 trainer 计算感知损失，将 (B, L, C) 转为 (B, C, H, W)
        # 这里转换 x_downsample 中的各阶段特征
        hierarchical_features = []
        H0, W0 = self.patches_resolution
        for i, feat in enumerate(x_downsample):
            Hi, Wi = H0 // (2 ** i), W0 // (2 ** i)
            B, L, C = feat.shape
            hierarchical_features.append(feat.view(B, Hi, Wi, C).permute(0, 3, 1, 2))

        # 3. 解码器恢复
        x_up = self.forward_up_features(x, x_downsample)

        # 4. 最终映射
        logits = self.up_x4(x_up)
